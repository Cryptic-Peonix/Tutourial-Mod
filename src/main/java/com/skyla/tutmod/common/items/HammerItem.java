...
public class HammerItem extends ToolItem {
	...
	public HammerItem(float attackDamageIn, float attackSpeedIn, IItemTier tier, Properties builderIn) {
		super((float) attackDamageIn, attackSpeedIn, tier, EFFECTIVE_ON,
				builderIn.addToolType(ModToolType.HAMMER, tier.getHarvestLevel()));
	}
	...
	public boolean hitEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		target.addPotionEffect(new EffectInstance(Effects.SLOWNESS, 50, 2));
		target.addPotionEffect(new EffectInstance(Effects.BLINDNESS, 25, 2));
